/**
 * FinanceExpenseList.tsx — Xarajatlar ro'yxati (web pages/finance/expenses/index.vue).
 * Oy navigatsiya + kategoriya stats + sana bo'yicha guruh + tahrirlash/o'chirish.
 */
...
import { catLabel, dateKeyOf, fDateHeader, fMoney, fTime, localDateKey, num, payLabel, sourceLabel } from './financeMoney';
...
const FinanceExpenseList = () => {
  ...
  const list = useFetch({
    url: `${URL}/finance/expenses?start_date=${startDate}&end_date=${endDate}&limit=500`,
    method: 'GET',
  });
  const stats = useFetch({
    url: `${URL}/finance/expenses/stats?year=${y}&month=${m}`,
    method: 'GET',
  });

  // MUHIM: barqaror onRefresh funksiyalariga bog'laymiz (butun `list`/`stats` obyektlari
  // har render'da yangi -> cheksiz refetch/abort loop bo'lardi). onRefresh = useState setter,
  // referensiyasi barqaror.
  ...
  // Sana bo'yicha guruh (kamayish).
  const groups = React.useMemo(() => {
    const map: Record<string, any[]> = {};
    items.forEach(it => {
      const key = dateKeyOf(it?.expense_date);
  ...
  const doDelete = async () => {
    if (!delTarget) return;
    try {
      await financeApi.deleteExpense(delTarget.id);
      setDelTarget(null);
      list.onRefresh({});
      stats.onRefresh({});
      Toast.show({ type: 'omad', props: { desc: 'Xarajat o‘chirildi' } });
    } catch (e) {
      setDelTarget(null);
  ...
  return (
    <View style={styles.screen}>
      ...
      <RdHeader title={t('Xarajatlar')} right={addBtn} backColor={RED} />
      ...
      {list.loading && !items.length ? (
        ...
        <ScrollView
          ...
          <View style={styles.banner}>
            ...
            <Text allowFontScaling={false} style={styles.bannerValue} numberOfLines={1} adjustsFontSizeToFit>
              {fMoney(statTotal)}
            </Text>
          ...
          {groups.length === 0 ? (
            <View style={styles.emptyBox}>
              ...
              <Text allowFontScaling={false} style={styles.emptyText}>{t('Bu oyda xarajatlar yo‘q')}</Text>
              <TouchableOpacity
                onPress={() => navigation.navigate('FinanceExpenseAdd')}
                style={styles.emptyBtn}
                activeOpacity={0.85}>
                <Text allowFontScaling={false} style={styles.emptyBtnText}>{t('Birinchi xarajatni qo‘shish')}</Text>
              </TouchableOpacity>
            </View>
          ) : (
            groups.map(g => (
                ...
                {g.items.map((it, i) => (
                  <View key={it?.id ?? i} style={[styles.row, i === 0 && { borderTopWidth: 0 }]}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {/* So'rov N13: manba (Sayt/Telegram/Mobil ilova). */}
                    ...
                    <View style={styles.rowRight}>
                      ...
                      <View style={styles.rowActions}>
                        <TouchableOpacity
                          onPress={() =>
                            navigation.navigate('FinanceExpenseAdd', { edit: it?.id })
                          }
      ...
      <Modal visible={!!delTarget} transparent animationType="fade" onRequestClose={() => setDelTarget(null)}>
        <View style={styles.modalWrap}>
          <View style={styles.modalCard}>
            <Text allowFontScaling={false} style={styles.modalTitle}>{t('Xarajatni o‘chirish')}</Text>
            <Text allowFontScaling={false} style={styles.modalMsg}>{t('Ushbu xarajatni o‘chirmoqchimisiz?')}</Text>
...
const styles = StyleSheet.create({
  ...
  groupTotal: { fontFamily: rd.font.bold, fontSize: rs(13), color: RED },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: rs(10),
  ...
  rowAmount: { fontFamily: rd.font.bold, fontSize: rs(13.5), color: RED },
  ...
  actEdit: { fontFamily: rd.font.semibold, fontSize: rs(11.5), color: RED },
  actDel: { fontFamily: rd.font.semibold, fontSize: rs(11.5), color: RED },
  ...
  modalDelete: { backgroundColor: RED },
